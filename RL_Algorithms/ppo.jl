#This script executes the Proximal Policy Optimisation Algorithm from Schulamnn
...
@everywhere env[:change_model](model="3D", prosthetic=true, difficulty=0)
sleep(1)
# =#
...
function ppo(params::Params, θa::Any, θv::Any, res::Array{Float64,1}, prec::Array{Float64,1}, all_traj::Traj_Data, best_net::Any, i_start)
    ...
    for i_iter = i_start:1:params.n_iter
        ...
        if floor(i_iter/5) == ceil(i_iter/5)
            ...
            run_episode_skeleton_demo!(params,θa,θa,all_traj.traj[1],rms_all,env)
        end
        #-----------------------------------------------------------------------
...
ppo(params, θa, θv, res, prec, all_traj, best_net,1)
